...
namespace App2 {
	class MatrixDialog : public Gtk::Dialog {
	private:
		//member variables
		Gtk::Grid grid;
	...
	public:
		...
		MatrixDialog(const Glib::ustring& title, Gtk::Window& parent, bool modal=false) :
			Gtk::Dialog(title, parent, modal) {
			BuildDialog();
		} 
		MatrixDialog(const Glib::ustring& title, Gtk::Window& parent, bool modal, std::vector<int> sample_elements) :
			Gtk::Dialog(title, parent, modal),
