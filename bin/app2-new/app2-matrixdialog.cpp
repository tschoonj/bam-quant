...
void App2::MatrixDialog::BuildDialog() {
	...
	//signals
	chemical_compound_radio.signal_toggled().connect(sigc::bind(sigc::mem_fun(*this, &App2::MatrixDialog::on_radio_toggled), &chemical_compound_radio));
	nist_compound_radio.signal_toggled().connect(sigc::bind(sigc::mem_fun(*this, &App2::MatrixDialog::on_radio_toggled), &nist_compound_radio));
	no_compound_radio.signal_toggled().connect(sigc::bind(sigc::mem_fun(*this, &App2::MatrixDialog::on_radio_toggled), &no_compound_radio));
	nist_compound_combobox.signal_changed().connect(sigc::mem_fun(*this, &App2::MatrixDialog::on_combobox_changed));
	chemical_compound_entry.signal_changed().connect(sigc::mem_fun(*this, &App2::MatrixDialog::on_entry_changed));
...
void App2::MatrixDialog::on_radio_toggled(Gtk::RadioButton *radio) {
	if (radio->get_active())
		on_radio_toggled_main();

}

void App2::MatrixDialog::on_radio_toggled_main() {
...
void App2::MatrixDialog::CheckCompound(std::string compound_string) {
	try {
...
void App2::MatrixDialog::on_combobox_changed() {
...
void App2::MatrixDialog::on_entry_changed() {
	if (!chemical_compound_radio.get_active()) {
